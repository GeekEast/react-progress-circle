...
const getPercentage = number => number > 100 ? 100 : (number < 0 ? 0 : number)
...
const reducer = (state, action) => {
  switch (action.type) {
    case 'SET_STROKE': return { ...state, strokeDasharray: action.payload }
    default: return state;
  }
}

const Progress = ({ width, height, strokeColor, isTextShown, number, children }) => {
  const [{ circle, svg, strokeDasharray }, dispatch] = useReducer(reducer, initialState);
  ...
  return (
    <div className="my-progress-container">
      <div>
        <svg className="my-progress-bar" viewBox={getViewBox(svg.viewPortWidth, svg.viewPortHeight)} width={width || svg.width} height={height || svg.height}>
          <circle className="circle-bg" {...circle}></circle>
          <circle className="circle-path" {...circle} style={{ 'strokeDasharray': strokeDasharray, 'stroke': strokeColor || '#5116d0' }}></circle>
          {
            isTextShown
            &&
            <text className="progress-text" x={circle.cx} y={circle.cy}>{getPercentage(number)} %</text>
          }
        </svg>
      </div>
